...
class Inference:
    ...
    def _load_meta(self):
        ...
        self.song_map = load_json('./arena_data/meta/song_map.json')
    ...
    def _predict_songlist(self, vt_list, vs_list):
        ...
        if len(relation_songs.index) > 0:
            ...
            for i in relation_songs.index:
                song_probability[i] += float(relation_songs.loc[i])
    ...
    def run(self):
        ...
        result = []
        for v in tqdm.tqdm(self.val[:1000]):
            predict = dict()
            predict['id'] = v['id']
